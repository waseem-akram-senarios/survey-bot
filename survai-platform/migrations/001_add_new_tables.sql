...
-- Run this against the existing database to add new tables and columns.
-- Safe to run multiple times (uses IF NOT EXISTS / IF NOT EXISTS checks).

-- ─── New Tables ──────────────────────────────────────────────────────────────

-- Campaigns: groups of surveys with scheduling
...
-- Riders: people who take surveys (for microtransit context)
...
-- Call transcripts: full recordings of agent calls
...
-- Survey analytics: post-survey AI analysis results
...
-- Templates: add configuration columns
DO $$
BEGIN
...
-- Surveys: add campaign and rider tracking
DO $$
BEGIN
...
-- Survey response items: add metadata columns
DO $$
BEGIN
