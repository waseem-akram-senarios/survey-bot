...
TEMPLATE_PROMPTS = {
    "Demo": {
        "start": (
            "Hi this is Cameron with {{OrganizationName}}, am I speaking to {{Recipient}}? "
            "If they say yes: Great! Do you have some time to walk through a brief survey? "
            "If they say yes: Perfect! [proceed to questions]. "
            "If they say no (to being the person): check if they can hand the phone to {{Recipient}}. "
            "If they say no (to time): Good to know, can we give you a call back at a later time? "
            "If yes to callback: ask what time works best. "
            "If no to callback: Can we email or text you the survey to fill out at your convenience? "
            "If yes to email: Great! We will send you the link. Thank you for your time and have a good rest of your day. "
            "If no to email: No problem! Have a great day."
        ),
        "verify_user": (
            "Hi this is Cameron with {{OrganizationName}}, am I speaking to {{Recipient}}? "
            "If not, ask if {{Recipient}} is available. "
        ),
        "closing": (
            "Thanks so much for your time and for sharing your thoughts, {{Recipient}}. "
...
class VAPISurveyWorkflowGenerator:
    ...
    def generate_workflow_from_survey(
        ...
        template_name = survey_data.get("Name", "Default")
        ...
        workflow = {
            ...
            #     "provider": "11labs",
            # },
            "globalPrompt": """You are an intelligent, conversational AI survey conductor.
        ...
        nodes, edges = self._create_nodes_and_edges(questions, survey_id, template_name)
        workflow["nodes"] = nodes
        workflow["edges"] = edges

        return workflow

    def _create_nodes_and_edges(self, questions: List[Dict], survey_id: str, template_name: str = "Default") -> tuple:
        ...
        start_node = self._create_start_node(template_name)
        nodes.append(start_node)

        # 2a. Create decline hangup node (conversation)
        decline_node_conv = self._create_decline_node_conv(template_name)
        ...
        end_node = self._create_end_node(questions, template_name)
        nodes.append(end_node)

        # 4. Create verify user node
        verify_user_node = self._create_verify_user_node(template_name)
    ...
    def _create_verify_user_node(self, template_name: str = "Default") -> Dict:
        """Create verify user node"""
        custom_prompts = TEMPLATE_PROMPTS.get(template_name, {})
        prompt_text = custom_prompts.get("verify_user", "You must get the name of the user. Ensure that he is indeed {{Recipient}}. If not ask them to hand over the phone to {{Recipient}}.")
    ...
    def _create_updatesurvey_node(self, questions: List[Dict], survey_id: str) -> Dict:
        ...
        return {
            ...
            "tool": {
                "url": "https://itcurves.duckdns.org/pg/api/surveys/{{SurveyId}}/update_survey_qna_phone",
    ...
    def _create_start_node(self, template_name: str = "Default") -> Dict:
        """Create the initial conversation node"""

        custom_prompts = TEMPLATE_PROMPTS.get(template_name, {})
        if "start" in custom_prompts:
             prompt_text = (
                "You are Cameron, a professional voice assistant. "
                "You are calling to conduct a survey. "
                f"Instructions: {custom_prompts['start']} "
                "Be polite, professional, and conversational. Capture the essence of the script but speak naturally."
             )
        else:
             prompt_text = (
                "You are Cameron, a professional survey collection voice assistant for IT Curves. "
                ...
                "Be polite, professional, and conversational. If they agree, proceed to the survey questions. If they explicitly refuse, proceed to end the call"
            )
        ...
        return {
            ...
            "prompt": prompt_text,
            "messagePlan": {
                "firstMessage": "Hello {{Recipient}}, this is Cameron calling on behalf of IT Curves."
            },
            "toolIds": [],
        }

    def _create_decline_node_conv(self, template_name: str = "Default") -> Dict:
    ...
    def _create_end_node(self, questions: List[Dict], template_name: str = "Default") -> Dict:
        """Create hangup node for survey completion"""
        # Note: In VAPI, endCall tool ends the call.
        # If we want a closing message, we should probably output it in the previous node (UpdateSurvey)
        # or add a conversation node before this. 
        # But the UpdateSurvey tool has a "request-start" message which acts as the closing.
        # We can update the UpdateSurvey node content if needed.
    ...
    def _create_categorical_prompt(
        ...
        prompt = (
            ...
            "If the user asks for more information, use speciifc, relevant parts of the following facts to address his queries. 1) This survey is being conducted on behalf of {{OrganizationName}} 2) This is for the Ride with ID {{RideID}}. 3) The goal of the survey is to improve the products and services based on user feedback."
        )
        return prompt
    ...
    def _create_open_prompt(self, question_text: str) -> str:
        ...
        prompt = (
            ...
            "If they give a very brief answer, you can ask follow-up questions like "
            "'Could you tell me a bit more about that?' to get more detailed feedback."
            "If they want to end the call or not proceed with the survey any further for any reason, do not question them further. "
            "If the user asks for more information, use speciifc, relevant parts of the following facts to address his queries. 1) This survey is being conducted on behalf of {{OrganizationName}} 2) This is for the Ride with ID {{RideID}}. 3) The goal of the survey is to improve the products and services based on user feedback."
        )
        return prompt
...
async def fetch_survey_data(survey_id):
    ...
    # Also fetch survey metadata to get the proper Survey Name / Template Name
    # We use sql_execute from utils directly here
    try:
        from utils import sql_execute
        sql_query = "SELECT name, template_name FROM surveys WHERE id = :survey_id"
        res = sql_execute(sql_query, {"survey_id": survey_id})
        if res:
            survey_name = res[0]["name"]
            # Inject name into survey_questions dict
            if survey_questions:
                survey_questions["Name"] = survey_name
    except Exception as e:
        logger.error(f"Error fetching survey name: {e}")
