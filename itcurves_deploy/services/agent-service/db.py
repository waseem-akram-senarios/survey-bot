...
Reads: surveys, survey_response_items, questions, templates, template_questions, riders
"""
...
def get_survey_with_questions(survey_id: str) -> Optional[Dict[str, Any]]:
    ...
    # Get questions with their order and categories
    ...
    # Get parent category mappings for branch questions
    for q in questions:
        if q.get("parent_id"):
...
def get_rider_data(rider_name: str = None, phone: str = None) -> Optional[Dict[str, Any]]:
    """Try to load rider data by name or phone."""
    if not rider_name and not phone:
        return None

    # Try riders table first (new)
    if phone:
        riders = sql_execute(
...
def get_template_config(template_name: str) -> Dict[str, Any]:
    """Get template configuration including time limits and restrictions."""
    ...
    # Default config if template not found or columns don't exist yet
    return {
        "name": template_name,
...
def store_transcript(
    ...
    # Update survey with call_id if provided
    if call_id:
        sql_execute(
...
def record_answer(survey_id: str, question_id: str, raw_answer: str) -> bool:
    """Record a survey answer from the agent's tool call."""
    try:
        sql_execute(
