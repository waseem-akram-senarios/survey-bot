...
const QuestionRenderer = ({
  ...
  const renderAutofillToggle = () => {
    ...
    if (!showAutofillToggle || !onAutofillToggle) return null;
    ...
    return (
      ...
      >
        ...
        <Switch
          ...
          onChange={(e) => onAutofillToggle(question.id, e.target.checked ? "Yes" : "No")}
          size="small"
          sx={{
      ...
      >
        ...
        >
          {ratingNumbers.map((value) => (
            <IconButton
              key={value}
              // Disable clicking for rating questions - they're display only
              onClick={undefined}
              sx={{
                width: isMobile ? "40px" : "52px",
                height: isMobile ? "40px" : "52px",
                ...
                borderRadius: "15px",
                backgroundColor:
                  question.rating >= value ? "#1958F7" : "#F8F8F8",
                color: question.rating >= value ? "#fff" : "#4B4B4B",
  ...
  if (question.type === "flow") {
    // Auto-select the first option that has child questions if no option is selected
        ...
        >
          ...
          <FormControl component="fieldset">
            ...
            >
              ...
              >
                {question.parentOptions?.map((option) => (
                  <FormControlLabel
                    key={option}
                    value={option}
                    control={<Radio sx={{ display: "none" }} />}
                    label={
                      <Chip
                        ...
                        sx={{
                          ...
                          fontSize: "14px",
                          border: "none",
                          cursor: "pointer", // Always clickable for flow questions
                          width: isMobile ? "100%" : "186px",
                          height: "37px",
                          justifyContent: "center",
                        }}
                        onClick={() => onUpdateSelection && onUpdateSelection(question.id, option)}
                      />
                    }
                    sx={{ m: 0 }}
                  />
